    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      <ScrollView contentContainerStyle={{ paddingVertical: 16 }}>
        <View className="px-4 pb-4">
          <Text className="text-2xl font-semibold text-slate-900">{input.query}</Text>
          <View className="flex-row items-center gap-3 mt-1">
            <Text className="text-xs text-slate-500 uppercase">{input.entityType}</Text>
            <Text className="text-xs text-slate-500">·</Text>
            <Text className="text-xs text-slate-500">{findings.length} finding{findings.length === 1 ? "" : "s"}</Text>
            <Text className="text-xs text-slate-500">·</Text>
            <SeverityPill severity={overall} />
          </View>
          {!anyLoaded ? <Text className="text-xs text-slate-400 mt-2">Searching…</Text> : null}
        </View>
